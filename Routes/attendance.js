...
router.get('/', objectIdMiddleware, asyncError(async (req, res, next) => {
    const { id } = req.params
...
router.post('/', objectIdMiddleware, validateAttendance, asyncError(async (req, res, next) => {
    const { id } = req.params
...
router.get('/new', objectIdMiddleware, asyncError(async (req, res, next) => {
    const { id } = req.params
    let notValidDates = []
    const singleClass = await Class.findById(id)
    .populate({
...
router.get('/:dateId', objectIdMiddleware, asyncError(async (req, res, next) => {
...
router.get('/:dateId/edit', objectIdMiddleware, asyncError(async (req, res, next) => {
    const { id, dateId } = req.params
    const singleClass = await Class.findById(id)
    .populate({
        path: 'studentsInClass',
...
router.put('/:dateId', objectIdMiddleware, asyncError(async (req, res, next) => {
    const { id, dateId } = req.params
...
router.delete('/:dateId', objectIdMiddleware, asyncError(async (req, res, next) => {
    const { id, dateId } = req.params
    const singleClass = await Class.findById(id)
